...
make_boot() {
	...
	cp ${work_dir}/root-image/usr/lib/syslinux/bios/isolinux.bin ${work_dir}/iso/isolinux/
        ...
        chroot ${work_dir}/boot-image dracut -fMN --no-early-microcode -c /etc/dracut.conf.d/dracut-live.conf /boot/kdeosiso.img --kver ${_kernver}
...
make_efiboot() {
    if [[ ! -e ${work_dir}/build.${FUNCNAME} ]]; then
        ...
        truncate -s 512M ${work_dir}/iso/EFI/kiso/kdeosiso.img
